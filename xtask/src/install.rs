//! Installs postgres_lsp language server and/or editor plugin.
...
fn install_client(sh: &Shell, client_opt: ClientOpt) -> anyhow::Result<()> {
    ...
    let installed_extensions = if cfg!(unix) {
        cmd!(sh, "{code} --install-extension circom-plus.vsix --force").run()?;
        cmd!(sh, "{code} --list-extensions").read()?
    } else {
        cmd!(sh, "cmd.exe /c {code}.cmd --install-extension circom-plus.vsix --force").run()?;
        cmd!(sh, "cmd.exe /c {code}.cmd --list-extensions").read()?
    };

    if !installed_extensions.contains("circom-plus") {
        bail!(
...
fn install_server(sh: &Shell) -> anyhow::Result<()> {
    let cmd = cmd!(sh, "cargo install --path crates/lsp --locked --force");
    cmd.run()?;
    Ok(())
}